...
function Sidebar() {
  ...
  return (
    <nav className='sidebar lg:hidden py-6 flex justify-start items-start fixed z-10 top-10 md:top-11 h-screen w-1/2 min-w-250 shadow-2xl shadow-zinc-400 bg-slate-50'>
      {error && <div className="absolute z-20 top-72 left-3 small animate-bounce bg-red-200 text-red-500 p-3 rounded-md">خطایی در خروج شما از حساب کاربری رخ داده است!</div>}
      {loading && <div className="absolute z-20 top-72 right-3 medium animate-bounce bg-white p-3 rounded-md shadow-2xl shadow-zinc-400">لطفا صبر نمایید...</div>}
      <ul className='w-full flex flex-col flex-wrap'>
        <li>
          <NavLink to='/' className='btn hover:btn-dark focus:ring-0 focus:ring-offset-0 rounded-none block w-full'>
            خانه<FontAwesomeIcon className='mr-1 smaller' icon={faHome} />
          </NavLink>
        </li>
        <li>
          <NavLink to='/about-us' className='btn hover:btn-dark focus:ring-0 focus:ring-offset-0 rounded-none block w-full'>
        ...
        {localStorage.getItem('userLoggedIn')?.length !== 4 && <ul className='w-full flex flex-col flex-wrap'>
          <li>
            <NavLink to='/sign-up' className='btn hover:btn-dark focus:ring-0 focus:ring-offset-0 rounded-none block w-full'>
              ثبت نام<FontAwesomeIcon className='mr-1 smaller' icon={faUserCheck} />
            </NavLink>
          </li>
          <li>
            <NavLink to='/sign-in' className='btn hover:btn-dark focus:ring-0 focus:ring-offset-0 rounded-none block w-full'>
